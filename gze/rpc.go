...
type RpcStubCreator[T interface{}] func(cli zrpc.Client) T
...
func NewRpcClientRouter[T interface{}](EtcdHosts []string, RpcKey string, creator RpcStubCreator[T]) *RpcClientRouter[T] {
...
func (r *RpcClientRouter[T]) AddRpcClient(ctx context.Context, KV *mvccpb.KeyValue) {
	...
	if _, ok := r.envStubMap[rpcKey.Env]; !ok {
		...
		r.envStubMap[rpcKey.Env] = r.creator(client)
	}
}
